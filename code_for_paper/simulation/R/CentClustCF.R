## Centralized cluster-robust causal forest
CentClustCF <- function(df, covars, honest=FALSE, is_pred=FALSE, myfit=NULL) {
    if (!is_pred) {
        myfit <- causal_forest(X = as.matrix(df[, ..covars]), Y=df$Y, W=df$Z,
                               honesty=honest, num.trees=nrow(df), clusters=df$S, equalize.cluster.weights=TRUE)
